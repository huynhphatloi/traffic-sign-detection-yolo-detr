...
Trọng số YOLOv8n & \texttt{yolov8n.pt} --- Ultralytics, tiền huấn luyện trên COCO \\
Trọng số DETR & \texttt{facebook/detr-resnet-50} --- Hugging Face Hub \\
Thư viện phát hiện & Ultralytics YOLO \cite{jocher2023ultralytics} \\
Thư viện transformer & Hugging Face Transformers \cite{wolf2020transformers} \\
Nền tảng học sâu & PyTorch \cite{paszke2019pytorch} \\
Môi trường huấn luyện & Google Colab, GPU đơn \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{lstlisting}[language=bash, caption={Các lệnh chạy lại từng bước của quy trình}, label={lst:cmds}]
...
# 4. Huan luyen va danh gia nhanh YOLO  (can GPU)
python -m src.training.train_yolo --model yolov8n.pt --epochs 30
python -m src.evaluation.evaluate_yolo --split test

# 5. Chuyen sang COCO, huan luyen va danh gia nhanh DETR  (can GPU)
python -m src.data.convert_to_coco
python -m src.training.train_detr
python -m src.evaluation.evaluate_detr --split test

# 6. Tong hop bang so sanh
python -m src.evaluation.compare_models

# 7. Chay bo kiem thu tu dong  (chi can CPU, khoang 1 giay)
pytest
\end{lstlisting}
...
Chỉ số YOLO                 & \texttt{results/metrics/yolo\_baseline.json} \\
Chỉ số DETR                 & \texttt{results/metrics/detr\_baseline.json} \\
Đường cong huấn luyện, PR, $F_1$, ma trận nhầm lẫn & Thư mục chạy của Ultralytics \\
Bảng thành phần bộ dữ liệu & Phân tích bổ sung theo quy ước đặt tên tệp \\
 Bảng và số liệu nhóm E3     & \texttt{notebooks/giaidoan2.ipynb} (đánh giá bằng pycocotools) \\
\bottomrule
\end{tabular}
\end{table}
...
trên môi trường Google Colab với GPU đơn tại thời điểm thực hiện đề tài.
